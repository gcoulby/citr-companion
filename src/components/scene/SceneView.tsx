...
function MysteryHeader() {
  ...
  return (
    <div className="flex items-center gap-4 bg-card/60 px-6 py-2.5 border-border border-b font-mono text-[11px] shrink-0">
      <span className="text-muted-foreground">
        Day <span className="text-foreground">{m.day}</span>
      </span>
      <span className="text-muted-foreground">
        Danger <span className="text-primary">{m.danger}</span>
      </span>
      <div className="flex items-center gap-1">
        ...
        {[0, 1, 2, 3].map((i) => (
          <span
            key={i}
            className={`w-2.5 h-2.5 rounded-full border ${i < m.clockMarks ? 'bg-primary/40 border-primary/60' : 'border-border'}`}
          />
        ))}
      </div>
      {m.problem.location && (
...
export function SceneView({ onSaved, onOpenDiceOracles, onOpenPlay }: Props) {
      ...
      <div className="flex flex-col flex-1 bg-background min-h-0 overflow-y-auto">
        <div className="space-y-3 p-6 max-w-md">
          ...
          <p className="text-[12px] text-muted-foreground">
            Start a mystery from the Mystery tab in the Play panel before
            playing a scene.
          </p>
  ...
  return (
    <div className="flex flex-col flex-1 bg-background min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeKind === 'obligation' && (
          <ObligationResolver onSaved={onSaved} />
        )}
        {activeKind === 'rest' && <RestResolver onSaved={onSaved} />}
        {activeKind === 'truth' && (
          <TruthResolver
            onSaved={onSaved}
            onOpenDiceOracles={onOpenDiceOracles}
          />
        )}
        {activeKind === 'investigation' && (
          <InvestigationResolver
            onSaved={onSaved}
            onOpenResolve={() => setActiveKind('resolve')}
          />
        )}
        {activeKind === 'resolve' && <ResolveResolver onSaved={onSaved} />}
      </div>
    </div>
  )
}
