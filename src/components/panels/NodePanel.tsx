...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-1.5 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">
      {children}
    </div>
  )
}
...
export function NodePanel({ nodeId, onClose, onOpenDocument }: Props) {
  ...
  const thumbnailUrl = node.thumbnail
    ? getCachedAsset(node.thumbnail)
    : undefined
  const mapImageUrl = mapImageAssetId
    ? getCachedAsset(mapImageAssetId)
    : undefined
  const mapSource = node.location
    ? resolveMapSource(
        mapStyle,
        customMapUrl,
  ...
  const removeAttachment = (id: string) => {
    updateNode(nodeId, {
      attachments: (node.attachments ?? []).filter((a) => a.id !== id),
    })
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div className="group relative">
              ...
              <div className="absolute inset-0 flex justify-center items-center gap-2 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className="bg-card px-3 py-1.5 border border-border hover:border-primary/60 rounded text-foreground text-xs"
                >
                  Replace
                </button>
                <button
                  onClick={() => updateNode(nodeId, { thumbnail: undefined })}
                  className="bg-card px-3 py-1.5 border border-red-400/30 hover:border-red-400/60 rounded text-red-400 text-xs"
                >
                  Remove
                </button>
              </div>
            ...
            <div
              ...
              className={[
                ...
                imgDragOver
                  ...
                  : 'border-border text-muted-foreground/70 hover:border-muted-foreground/40 hover:text-muted-foreground',
              ].join(' ')}
            >
              <ImagePlus size={18} />
        ...
        <div className="space-y-4 p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                ...
                className={[
                  ...
                  !node.nodeType
                    ? 'border-muted-foreground/40 text-foreground bg-muted'
          ...
          {(node.clue || node.truth || node.threat) && (
            <div className="space-y-2 bg-background px-3 py-2.5 border border-border rounded text-[11px]">
              {node.clue &&
                (() => {
              ...
              {node.truth && (
                <div className="flex items-start gap-2">
                  {node.truth.card && (
                    <PlayingCardView card={node.truth.card} size="sm" />
                  )}
                  <div className="font-mono text-yellow-300">
                    {node.truth.connection || 'Confirmed truth'}
                  </div>
                </div>
              )}
              {node.threat && (
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updateNode(nodeId, {
                          threat: { ...node.threat!, kind },
                        })
                        if (node.threat!.threatId)
                          updateThreat(node.threat!.threatId, { kind })
                      }}
                      className="bg-background px-1.5 py-0.5 border border-border rounded font-mono text-[10px] text-foreground"
                    >
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        updateNode(nodeId, {
                          threat: { ...node.threat!, level },
                        })
                        if (node.threat!.threatId)
                          updateThreat(node.threat!.threatId, { level })
                      }}
                      className="bg-background px-1.5 py-0.5 border border-border rounded font-mono text-[10px] text-foreground"
                    >
                  ...
                  <div className="text-[10px] text-muted-foreground/70 normal-case">
                    Name is set via the Label field above
                    {node.threat.threatId
                      ? ' and stays in sync with the Play panel'
                      : ''}
                    .
                  </div>
                </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (node.threat?.threatId)
                  renameThreat(node.threat.threatId, e.target.value)
              }}
            />
          </div>
          ...
          <div>
            ...
            <button
              ...
              className="group flex items-center gap-2.5 hover:bg-primary/5 px-3 py-2.5 border border-border hover:border-primary/40 rounded w-full text-left transition-colors"
            >
              <BookOpen
                size={14}
                className={
              ...
              <div>
                <div
                  className={`text-[12px] font-medium ${node.hasContent ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'}`}
                >
            ...
            <div>
              ...
              <div className="space-y-1">
                {backlinks.map(({ docId }) => {
                  ...
                  const sourceLabel = isCaseNotes
                    ? 'Case Notes'
                    : (allNodes[docId]?.label ?? 'Untitled')
                  return (
                    <button
                      key={docId}
                      onClick={() =>
                        onOpenDocument(
                          isCaseNotes
                            ? { kind: 'case' }
                            : { kind: 'node', nodeId: docId },
                        )
                      }
                      className="flex items-center gap-2 hover:bg-primary/5 px-2.5 py-1.5 border border-border hover:border-primary/40 rounded w-full text-left transition-colors"
                    >
                      {isCaseNotes ? (
                        <NotebookPen
                          size={11}
          ...
          <div>
            ...
            {node.location ? (
              <div className="space-y-2">
                <div className="flex items-start gap-2 bg-background px-2 py-2 border border-border rounded">
                  <MapPin size={12} className="mt-0.5 text-primary shrink-0" />
                  <div className="flex-1 min-w-0">
                    {node.location.label && (
                      <div className="mb-0.5 text-[11px] text-foreground truncate leading-tight">
                        {node.location.label}
                      </div>
                    )}
                    <div className="font-mono text-[10px] text-muted-foreground/80">
                      {node.location.lat.toFixed(5)},{' '}
                      {node.location.lng.toFixed(5)}
                    </div>
                  </div>
                  <button
                ...
                {mapSource && (
                  <div>
                    <div
                      className="relative border border-border rounded h-24 overflow-hidden"
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5">
                      <span className="mr-auto font-mono text-[10px] text-muted-foreground/70 uppercase tracking-wider">
                        Preview zoom
                      </span>
                      {Boolean(node.location.zoomOffset) && (
                        <button
                          onClick={() =>
                            updateNode(nodeId, {
                              location: {
                      ...
                      <button
                        onClick={() =>
                          updateNode(nodeId, {
                            location: {
                              ...node.location!,
                              zoomOffset: (node.location!.zoomOffset ?? 0) - 1,
                            },
                          })
                        }
                        className="flex justify-center items-center border border-border hover:border-primary/40 rounded w-5 h-5 text-muted-foreground hover:text-primary transition-colors"
                        title="Zoom out"
                      >
                        <Minus size={10} />
                      </button>
                ...
                {node.thumbnail && (
                  <div>
                    <div className="mb-1 font-mono text-[10px] text-muted-foreground/70 uppercase tracking-wider">
                      Card Feature
                    </div>
                    <div className="flex gap-1.5">
                      <button
                        onClick={() =>
                          updateNode(nodeId, { featureDisplay: 'image' })
                        }
                        className={[
                      ...
                      <button
                        onClick={() =>
                          updateNode(nodeId, { featureDisplay: 'map' })
                        }
                        className={[
                          'flex items-center gap-1.5 px-2.5 py-1 rounded border text-[10px] transition-colors',
                ...
                <div key={k} className="flex items-center gap-1.5">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      delete props[k]
                      props[e.target.value] = v
                      updateNode(nodeId, { properties: props })
                    }}
                    className="w-24 h-7 font-mono text-[10px] text-muted-foreground"
                  />
                  <Input
                    value={v}
                    onChange={(e) =>
                      updateNode(nodeId, {
                        properties: { ...node.properties, [k]: e.target.value },
                      })
                    }
                  ...
                  <button
                    onClick={() => {
                      ...
                      delete p[k]
                      updateNode(nodeId, { properties: p })
                    }}
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && newPropKey.trim()) {
                    updateNode(nodeId, {
                      properties: {
                        ...node.properties,
                        [newPropKey.trim()]: newPropVal.trim(),
                      },
                    })
                    setNewPropKey('')
                    setNewPropVal('')
          ...
          <div>
            ...
            {(node.attachments ?? []).length === 0 ? (
              <div className="mb-2 text-[11px] text-muted-foreground/40">
                No files attached
              </div>
            ) : (
              <div className="space-y-1 mb-2">
                {(node.attachments ?? []).map((att) => (
                  <div
                    key={att.id}
                    className="group flex items-center gap-2 bg-background px-2 py-1.5 border border-border rounded"
                  >
                    <FileText
                      size={10}
                      className="text-muted-foreground/70 shrink-0"
                    ...
                    <button
                      ...
                      className="flex-1 text-[11px] text-muted-foreground hover:text-foreground text-left truncate transition-colors"
                    >
                      {att.filename}
                    </button>
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 text-muted-foreground/70 hover:text-red-400 transition-colors"
                    >
                      <X size={10} />
                    </button>
                  </div>
          ...
          {/* Timestamps */}
          <div className="space-y-0.5 pb-2 font-mono text-[10px] text-muted-foreground/40">
      ...
      {/* Delete */}
      <div className="p-3 border-border border-t shrink-0">
        <Button
          variant="destructive"
          className="w-full"
          onClick={() => {
