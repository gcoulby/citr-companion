...
function RollLabel({
  ...
  return (
    <div className="flex justify-between items-center mb-1.5">
      ...
      <button
        onClick={onRoll}
        title="Roll for inspiration"
        className="-mt-1.5 text-muted-foreground/60 hover:text-primary transition-colors"
      >
        <Dices size={12} />
      </button>
    </div>
  ...
  return (
    <div className="space-y-5 p-4">
      <div>
        ...
        {portraitUrl ? (
          <div className="group relative border border-border rounded-lg w-20 h-20 overflow-hidden">
            <img
              src={portraitUrl}
              alt=""
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 flex justify-center items-center gap-1.5 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
              ...
              <button
                onClick={() => inv.setPortrait(undefined)}
                title="Remove"
      ...
      <div>
        <RollLabel
          onRoll={() =>
            inv.setName(
              `${rollOracleTable(FIRST_NAME_TABLE).result} ${rollOracleTable(LAST_NAME_TABLE).result}`,
            )
          }
        >
          Name
        ...
        <TextInput
          value={inv.name}
          onChange={(e) => inv.setName(e.target.value)}
      ...
      <div>
        <RollLabel
          onRoll={() => inv.setTrait(rollOracleTable(TRAIT_TABLE).result)}
        >
          Trait
        </RollLabel>
        <TextInput
          value={inv.trait}
          onChange={(e) => inv.setTrait(e.target.value)}
          placeholder="e.g. coldly pragmatic…"
        />
      </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1">
            {[0, 1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className={`w-6 h-6 rounded border ${i < inv.fatigue ? 'bg-red-400/20 border-red-400/50' : 'border-border'}`}
              />
            ))}
          </div>
      ...
      <div>
        <RollLabel
          onRoll={() =>
            setNewObligation(rollOracleTable(genreTables.obligations).result)
          }
        >
          Obligations
        </RollLabel>
        <div className="space-y-1 mb-2">
          {inv.obligations.map((o) => (
            ...
            >
              ...
              <button
                onClick={() => inv.removeObligation(o.id)}
        ...
        <div className="flex gap-1.5">
          <TextInput
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && newObligation.trim()) {
                inv.addObligation(newObligation.trim())
                setNewObligation('')
              }
            }}
          />
      ...
      <div>
        <RollLabel
          onRoll={() =>
            setNewKeyword(rollOracleTable(genreTables.keywords).result)
          }
        >
          Keywords
        </RollLabel>
        <div className="space-y-1 mb-2">
          {inv.keywords.map((k) => (
            ...
            >
              <span
                className={`flex-1 text-[12px] ${k.struck ? 'text-muted-foreground/70 line-through' : 'text-foreground'}`}
              >
                {k.text}
              </span>
              {k.signature && <Badge tone="amber">signature</Badge>}
              {!k.struck && (
                <SmallButton onClick={() => inv.useKeyword(k.id)}>
                  Use
                </SmallButton>
              )}
              <button
                onClick={() => inv.removeKeyword(k.id)}
                className="opacity-0 group-hover:opacity-100 text-muted-foreground/70 hover:text-red-400 transition-opacity"
              >
                <X size={11} />
              </button>
          ...
          <TextInput
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && newKeyword.trim()) {
                inv.addKeyword(newKeyword.trim(), newKeywordSignature)
                setNewKeyword('')
                setNewKeywordSignature(false)
              }
            }}
          />
          <label className="flex items-center gap-1 text-[10px] text-muted-foreground whitespace-nowrap shrink-0">
            <input
              type="checkbox"
              checked={newKeywordSignature}
              onChange={(e) => setNewKeywordSignature(e.target.checked)}
            />
            signature
          </label>
          <button
